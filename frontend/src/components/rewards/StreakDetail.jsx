...
export default function StreakDetail({ rewards }) {
  ...
  return (
    <div style={{ animation: 'fadeInUp 0.25s ease-out' }}>
      ...
      <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(78,205,196,0.1)', padding: 20, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            ...
            <div style={{ ...fp, fontSize: 8, color: 'rgba(200,168,75,0.6)', letterSpacing: 1, marginTop: 4 }}>DAY STREAK</div>
          </div>
          {currentStreakTier && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 16px', border: `1px solid ${GOLD}`, background: 'rgba(0,0,0,0.2)' }}>
              ...
              <div>
                <div style={{ ...fp, fontSize: 9, color: GOLD, letterSpacing: 1 }}>{currentStreakTier.name}</div>
                <div style={{ ...fp, fontSize: 8, color: 'rgba(200,168,75,0.5)', marginTop: 2 }}>GOLD FRAME</div>
              </div>
            </div>
          )}
        </div>
        ...
        {nextStreakTier && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <div style={{ ...fp, fontSize: 8, color: 'rgba(255,255,255,0.4)', letterSpacing: 1 }}>NEXT: {nextStreakTier.name} ({nextStreakTier.days} DAYS)</div>
              <div style={{ ...fp, fontSize: 8, color: GOLD, letterSpacing: 1 }}>{streak} / {nextStreakTier.days}</div>
            </div>
        ...
        <div style={{ display: 'flex', gap: 0, marginTop: 20 }}>
          {timeline.map((day, i) => (
            <div key={i} style={{
              ...
              <div style={{ ...fp, fontSize: 8, color: day.isToday ? 'rgba(200,168,75,0.6)' : 'rgba(255,255,255,0.3)', marginBottom: 4 }}>{day.label}</div>
              {day.done && !day.isToday && <div style={{ ...fv, fontSize: 20, color: REWARDS_ACCENT }}>✓</div>}
              {day.isToday && <div style={{ ...fv, fontSize: 20, color: GOLD, animation: 'goldPulse 2s infinite' }}>⬡</div>}
              {!day.done && !day.isToday && <div style={{ ...fv, fontSize: 20, color: 'rgba(255,255,255,0.1)' }}>·</div>}
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Streak tiers */}
      <div style={{ ...fp, fontSize: 8, color: 'rgba(255,255,255,0.5)', letterSpacing: 1, marginBottom: 12 }}>STREAK TIERS</div>
        ...
        {streakTiers.map((tier, i) => {
          ...
          return (
            <div key={i} style={cellStyle}>
              ...
              <div style={{ ...fp, fontSize: 8, color: locked ? 'rgba(255,255,255,0.4)' : (isCurrent ? GOLD : tier.color) }}>{tier.name}</div>
              <div style={{ ...fv, fontSize: 16, color: isCurrent ? GOLD : (locked ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.3)') }}>{tier.days} days</div>
              {earned && !isCurrent && <div style={{ ...fp, fontSize: 8, color: 'rgba(78,205,196,0.6)', marginTop: 3 }}>✓</div>}
              {isCurrent && <div style={{ ...fp, fontSize: 8, color: GOLD, marginTop: 3 }}>◆ NOW</div>}
              {tier.permanent && locked && <div style={{ ...fp, fontSize: 8, color: 'rgba(200,168,75,0.3)', marginTop: 3 }}>PERMANENT</div>}
            </div>
          )
        })}
      </div>
