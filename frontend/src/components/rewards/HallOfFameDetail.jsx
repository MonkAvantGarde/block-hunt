...
export default function HallOfFameDetail({ rewards, onClaim }) {
  ...
  function claimBtn(item) {
    ...
    return (
      <button
        ...
        style={{
          ...fp, fontSize: 8, letterSpacing: 1, padding: '4px 10px',
  ...
  return (
    <div style={{ animation: 'fadeInUp 0.25s ease-out' }}>
      {/* Section: Legends */}
      <div style={{ ...fp, fontSize: 9, color: GOLD_LT, letterSpacing: 2, marginBottom: 4, textShadow: '0 0 12px rgba(200,168,75,0.3)' }}>LEGENDS — ALL TIME</div>
      <div style={{ ...fp, fontSize: 8, color: 'rgba(255,255,255,0.25)', letterSpacing: 1, marginBottom: 14 }}>ONCE CLAIMED, FOREVER YOURS</div>
      ...
      {hallOfFame.legends.map((legend, i) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ ...fp, fontSize: 8, color: claimed ? GOLD_LT : 'rgba(200,168,75,0.35)', letterSpacing: 1, flex: 1 }}>
              {legend.title} — {legend.desc}
            </div>
            {claimed ? (
              <>
                <div style={{ ...fv, fontSize: 20, color: 'rgba(255,255,255,0.4)' }}>
                  {legend.wallet}{legend.isYou ? <span style={{ ...fp, fontSize: 8, color: GOLD, marginLeft: 4 }}>YOU</span> : null}
                </div>
                {legend.isYou && !legend.claimed ? claimBtn(legend) : (
                  <div style={{ ...fv, fontSize: 20, color: GOLD_LT, minWidth: 60, textAlign: 'right' }}>TITLE</div>
                )}
              </>
            ) : (
              <>
                <div style={{ ...fv, fontSize: 20, color: 'rgba(255,255,255,0.12)' }}>UNCLAIMED</div>
                <div style={{ ...fp, fontSize: 8, color: 'rgba(255,255,255,0.25)', minWidth: 60, textAlign: 'right' }}>AVAILABLE</div>
              </>
            )}
          </div>
        )
      ...
      <div style={{ ...fp, fontSize: 8, color: 'rgba(200,168,75,0.5)', letterSpacing: 1, marginTop: 20, marginBottom: 4 }}>TIER DISCOVERY — ALL TIME</div>
      <div style={{ ...fp, fontSize: 8, color: 'rgba(255,255,255,0.2)', letterSpacing: 1, marginBottom: 12 }}>FIRST WALLET TO REVEAL EACH TIER</div>
        ...
        return (
          ...
          >
            ...
            {claimed ? (
              <>
                <div style={{ ...fv, fontSize: 20, color: 'rgba(255,255,255,0.4)' }}>
                  {td.wallet}{td.isYou ? <span style={{ ...fp, fontSize: 8, color: GOLD, marginLeft: 4 }}>YOU</span> : null}
                </div>
      ...
      <div style={{ marginTop: 24 }}>
        <div style={{ display: 'inline-block', padding: '3px 8px', border: '1px solid rgba(78,205,196,0.2)', background: 'rgba(78,205,196,0.04)', marginBottom: 12 }}>
          <span style={{ ...fp, fontSize: 8, color: 'rgba(78,205,196,0.6)', letterSpacing: 1 }}>BATCH {batch} FIRSTS — ETH BONUS + TITLE (13 ACHIEVEMENTS)</span>
        </div>
        ...
        {hallOfFame.batchFirsts.map((bf, i) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ ...fp, fontSize: 8, color: haswinner ? REWARDS_ACCENT : 'rgba(78,205,196,0.4)', letterSpacing: 1, flex: 1 }}>{bf.title}</div>
              {haswinner ? (
                <>
                  <div style={{ ...fv, fontSize: 20, color: 'rgba(255,255,255,0.4)' }}>
                    {bf.wallet}{bf.isYou ? <span style={{ ...fp, fontSize: 8, color: GOLD, marginLeft: 4 }}>YOU</span> : null}
                  </div>
                  {bf.isYou && !bf.claimed ? claimBtn(bf) : (
                    <div style={{ ...fv, fontSize: 20, color: REWARDS_ACCENT, minWidth: 70, textAlign: 'right' }}>+{bf.prize.toFixed(2)} Ξ{bf.claimed ? ' ✓' : ''}</div>
                  )}
                </>
              ) : (
                <>
                  <div style={{ ...fv, fontSize: 20, color: 'rgba(255,255,255,0.1)' }}>UNCLAIMED</div>
                  <div style={{ ...fp, fontSize: 8, color: 'rgba(255,255,255,0.25)', minWidth: 70, textAlign: 'right' }}>{bf.prize.toFixed(2)} Ξ</div>
                </>
              )}
            </div>
          )
        ...
        <div style={{ marginTop: 14, marginBottom: 10, paddingTop: 12, borderTop: '1px solid rgba(78,205,196,0.06)' }}>
          <div style={{ ...fp, fontSize: 8, color: 'rgba(78,205,196,0.5)', letterSpacing: 1 }}>BATCH {batch} TIER DISCOVERY</div>
        </div>
        ...
        {hallOfFame.batchTierDiscovery.map((btd, i) => {
          ...
          const rankBoxStyle = haswinner
            ? { background: tierData?.bg || 'rgba(0,0,0,0.3)', color: tierData?.accent || '#fff', border: `1px solid ${tierData?.border || 'rgba(255,255,255,0.1)'}`, fontSize: 18 }
            : { background: 'rgba(0,0,0,0.3)', color: `${tierData?.accent || '#fff'}66`, border: `1px solid ${tierData?.border || 'rgba(255,255,255,0.1)'}50`, fontSize: 18 }
          ...
          return (
            ...
            >
              ...
              <div style={{ ...fp, fontSize: 8, color: haswinner ? REWARDS_ACCENT : 'rgba(78,205,196,0.4)', letterSpacing: 1, flex: 1 }}>
                FIRST <span style={{ color: haswinner ? (tierData?.accent || '#fff') : `${tierData?.accent || '#fff'}66` }}>{btd.name}</span> IN BATCH {batch}
              </div>
              {haswinner ? (
                <>
                  <div style={{ ...fv, fontSize: 20, color: 'rgba(255,255,255,0.4)' }}>
                    {btd.wallet}{btd.isYou ? <span style={{ ...fp, fontSize: 8, color: GOLD, marginLeft: 4 }}>YOU</span> : null}
                  </div>
                  {btd.isYou && !btd.claimed ? claimBtn({ ...btd, title: `FIRST ${btd.name} IN BATCH ${batch}` }) : (
                    <div style={{ ...fv, fontSize: 20, color: REWARDS_ACCENT, minWidth: 70, textAlign: 'right' }}>+{btd.prize.toFixed(2)} Ξ{btd.claimed ? ' ✓' : ''}</div>
                  )}
                </>
              ) : (
                <>
                  <div style={{ ...fv, fontSize: 20, color: 'rgba(255,255,255,0.1)' }}>UNCLAIMED</div>
                  <div style={{ ...fp, fontSize: 8, color: 'rgba(255,255,255,0.25)', minWidth: 70, textAlign: 'right' }}>{btd.prize.toFixed(2)} Ξ</div>
                </>
              )}
            </div>
          )
