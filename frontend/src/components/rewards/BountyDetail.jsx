    ...
    <div style={{ animation: 'fadeInUp 0.25s ease-out' }}>
      ...
      <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(78,205,196,0.1)', padding: 20, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            <div style={{ ...fp, fontSize: 9, color: REWARDS_ACCENT, letterSpacing: 2, textShadow: '0 0 12px rgba(78,205,196,0.3)' }}>BATCH {bounty.currentBatch} BOUNTY</div>
            <div style={{ ...fp, fontSize: 5, color: 'rgba(255,255,255,0.3)', marginTop: 4, letterSpacing: 1 }}>EVERYONE WHO MINTED SHARES THE PRIZE</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ ...fp, fontSize: 5, color: 'rgba(78,205,196,0.5)', letterSpacing: 1 }}>TOTAL BOUNTY</div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <div style={{ ...fp, fontSize: 7, color: 'rgba(255,255,255,0.6)', letterSpacing: 1 }}>BATCH {bounty.currentBatch} PROGRESS</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
            <div style={{ ...fp, fontSize: 5, color: 'rgba(255,255,255,0.25)' }}>{remaining.toLocaleString()} BLOCKS REMAINING</div>
            <div style={{ ...fp, fontSize: 5, color: REWARDS_ACCENT, animation: 'progressPulse 1.5s infinite' }}>{pct}%</div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, border: '1px solid rgba(78,205,196,0.08)' }}>
          <div style={{ padding: 12, textAlign: 'center', background: 'rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ ...fp, fontSize: 5, color: 'rgba(255,255,255,0.35)', letterSpacing: 1, marginTop: 2 }}>ELIGIBLE WALLETS</div>
          </div>
          <div style={{ padding: 12, textAlign: 'center', background: 'rgba(0,0,0,0.2)', borderLeft: '1px solid rgba(78,205,196,0.08)' }}>
            ...
            <div style={{ ...fp, fontSize: 5, color: 'rgba(255,255,255,0.35)', letterSpacing: 1, marginTop: 2 }}>EST. PER WALLET</div>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ ...fv, fontSize: 22, color: bounty.userEligible ? GREEN : 'rgba(255,255,255,0.2)' }}>{bounty.userEligible ? '✓' : '✗'}</div>
      ...
      {bounty.completedBatches.map((batch, i) => (
        <div key={i} style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(78,205,196,0.06)', padding: 16, opacity: 0.6 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div>
              <div style={{ ...fp, fontSize: 8, color: 'rgba(78,205,196,0.6)', letterSpacing: 1 }}>BATCH {batch.batch} BOUNTY</div>
              <div style={{ ...fp, fontSize: 5, color: 'rgba(255,255,255,0.2)', marginTop: 3 }}>COMPLETE — {batch.total.toLocaleString()} / {batch.total.toLocaleString()}</div>
            </div>
          ...
          {batch.claimed && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: 'rgba(200,168,75,0.06)', border: '1px solid rgba(200,168,75,0.15)' }}>
              <div style={{ ...fp, fontSize: 6, color: GOLD, letterSpacing: 1 }}>CLAIMED — +{batch.claimedAmount} Ξ</div>
              <div style={{ ...fv, fontSize: 16, color: GOLD }}>✓</div>
            </div>
          )}
        </div>
      ))}
