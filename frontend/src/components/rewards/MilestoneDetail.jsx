  ...
  return (
    <div style={{ animation: 'fadeInUp 0.25s ease-out' }}>
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 20 }}>
        <button style={{ ...fp, flex: 1, padding: 10, fontSize: 8, letterSpacing: 1, color: REWARDS_ACCENT, background: 'rgba(78,205,196,0.08)', border: '1px solid rgba(78,205,196,0.2)', borderBottom: `2px solid ${REWARDS_ACCENT}`, cursor: 'pointer' }}>OVERALL</button>
        <button style={{ ...fp, flex: 1, padding: 10, fontSize: 8, letterSpacing: 1, color: 'rgba(255,255,255,0.4)', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.06)', borderBottom: '2px solid transparent', cursor: 'pointer' }}>BATCH 2</button>
      </div>
        ...
        return (
          <div key={key} style={{ marginBottom: 12, animation: 'fadeInUp 0.3s ease-out' }}>
            ...
            <div style={{
              ...
              <div style={{ ...fv, fontSize: 24, color: cat.color, width: 28, textAlign: 'center' }}>{cat.icon}</div>
              <div style={{ ...fp, fontSize: 8, color: 'rgba(255,255,255,0.7)', letterSpacing: 1, flex: 1 }}>{cat.label}</div>
              <div style={{ ...fv, fontSize: 20, color: REWARDS_ACCENT }}>{earned} / {cat.badges.length}</div>
            </div>
              ...
              {cat.badges.map((badge, i) => {
                ...
                return (
                  <div key={i} style={badgeStyle}>
                    ...
                    <div style={{ ...fp, fontSize: 8, color: isEarned ? REWARDS_ACCENT : (isNext ? GOLD : 'rgba(255,255,255,0.4)'), marginBottom: 2 }}>{badge.name}</div>
                    ...
                    {isNext && (
                      <div style={{ marginTop: 5, paddingTop: 5, borderTop: '1px solid rgba(200,168,75,0.15)' }}>
                        <div style={{ ...fp, fontSize: 8, color: 'rgba(200,168,75,0.6)' }}>{cat.current.toLocaleString()} / {badge.count.toLocaleString()}</div>
