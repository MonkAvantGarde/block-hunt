  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <Card onClick={() => onNavigate('streak')} delay={0}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ ...fp, fontSize: 6, color: 'rgba(255,255,255,0.4)', letterSpacing: 1 }}>DAILY STREAK</div>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            ...
            <div>
              <div style={{ ...fp, fontSize: 6, color: 'rgba(200,168,75,0.6)' }}>DAYS</div>
              {currentStreakTier && (
                <div style={{ ...fp, fontSize: 6, color: GOLD, marginTop: 3 }}>{currentStreakTier.name} ✦</div>
              )}
            </div>
          </div>
          {nextStreakTier && (
            <div style={{ marginTop: 10 }}>
              <div style={{ ...fp, fontSize: 5, color: 'rgba(255,255,255,0.3)', marginBottom: 3 }}>NEXT: {nextStreakTier.name} ({nextStreakTier.days} days)</div>
        ...
        <Card onClick={() => onNavigate('lottery')} delay={0.05}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ ...fp, fontSize: 6, color: 'rgba(255,255,255,0.4)', letterSpacing: 1 }}>TODAY'S LOTTERY</div>
            <div className="card-arrow" style={{ ...fp, fontSize: 6, color: REWARDS_ACCENT, opacity: 0, transition: 'opacity 0.2s' }}>→</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            ...
            <div style={{ ...fv, fontSize: 22, color: 'rgba(78,205,196,0.6)' }}>Ξ</div>
          </div>
        ...
        <Card onClick={() => onNavigate('milestones')} delay={0.1}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ ...fp, fontSize: 6, color: 'rgba(255,255,255,0.4)', letterSpacing: 1 }}>MILESTONES</div>
            <div className="card-arrow" style={{ ...fp, fontSize: 6, color: REWARDS_ACCENT, opacity: 0, transition: 'opacity 0.2s' }}>→</div>
          </div>
          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 8 }}>
            {earnedBadges.slice(-2).map((b, i) => (
              <div key={i} style={{ padding: '3px 7px', background: 'rgba(78,205,196,0.1)', border: '1px solid rgba(78,205,196,0.2)' }}>
                <span style={{ ...fp, fontSize: 5, color: REWARDS_ACCENT }}>{b.name}</span>
              </div>
            ))}
            {nextBadge && (
              <div style={{ padding: '3px 7px', background: 'rgba(200,168,75,0.1)', border: '1px solid rgba(200,168,75,0.2)' }}>
                <span style={{ ...fp, fontSize: 5, color: GOLD }}>{nextBadge.name} ◇</span>
              </div>
            )}
          </div>
          <div style={{ ...fp, fontSize: 5, color: 'rgba(255,255,255,0.3)' }}>{totalEarned} EARNED · {totalInProgress} IN PROGRESS</div>
        </Card>

        {/* BATCH BOUNTY CARD */}
        <Card onClick={() => onNavigate('bounty')} delay={0.15}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ ...fp, fontSize: 6, color: 'rgba(255,255,255,0.4)', letterSpacing: 1 }}>BATCH {bounty.currentBatch} BOUNTY</div>
            <div className="card-arrow" style={{ ...fp, fontSize: 6, color: REWARDS_ACCENT, opacity: 0, transition: 'opacity 0.2s' }}>→</div>
          </div>
          <div style={{ marginBottom: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              <div style={{ ...fp, fontSize: 5, color: 'rgba(255,255,255,0.3)' }}>{bounty.minted.toLocaleString()} / {bounty.total.toLocaleString()}</div>
              <div style={{ ...fv, fontSize: 16, color: '#f0ead6' }}>{Math.round((bounty.minted / bounty.total) * 100)}%</div>
            </div>
          ...
          <div style={{ ...fp, fontSize: 5, color: REWARDS_ACCENT }}>{bounty.bountyAmount.toFixed(2)} Ξ BOUNTY · {bounty.userEligible ? "YOU'RE ELIGIBLE" : 'MINT TO QUALIFY'}</div>
        </Card>

        {/* HALL OF FAME CARD — full width */}
        <Card onClick={() => onNavigate('hof')} delay={0.2} style={{ gridColumn: '1 / -1' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ ...fp, fontSize: 6, color: 'rgba(255,255,255,0.4)', letterSpacing: 1 }}>HALL OF FAME</div>
            <div className="card-arrow" style={{ ...fp, fontSize: 6, color: REWARDS_ACCENT, opacity: 0, transition: 'opacity 0.2s' }}>→</div>
          </div>
            ...
            {hallOfFame.legends.filter(l => l.wallet).slice(0, 2).map((l, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ width: 24, height: 24, background: `linear-gradient(135deg,${GOLD_DK},${GOLD})`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ ...fv, fontSize: 16, color: '#0a0705' }}>★</span>
                </div>
                <div>
                  <div style={{ ...fp, fontSize: 5, color: GOLD_LT }}>{l.title}</div>
                  <div style={{ ...fv, fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>{l.wallet}{l.isYou ? ' (YOU)' : ''}</div>
                </div>
              </div>
            ))}
      ...
      <div style={{ textAlign: 'center', marginTop: 16, animation: 'fadeIn 0.5s ease-out 0.4s both' }}>
        <div style={{ ...fp, fontSize: 6, color: 'rgba(78,205,196,0.35)', letterSpacing: 2 }}>TAP ANY CARD TO LEARN MORE →</div>
      </div>
    </div>
  )
}
