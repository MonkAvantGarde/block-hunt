...
import {
  ...
  trimLeft,
  validate
} from "./chunk-B4V6ZMUC.js";
import "./chunk-6QNFSXSS.js";
import {
  __export
...
function verify2(options) {
  ...
  return secp256r12.verify(signature, payload instanceof Uint8Array ? payload : fromHex(payload), toHex3(publicKey).substring(2), { lowS: true, ...hash4 ? { prehash: true } : {} });
}
...
__export(TempoAddress_exports, {
  ...
  InvalidLengthError: () => InvalidLengthError2,
...
function format(address, options = {}) {
  ...
  const zone = zoneId != null ? toBytes5(zoneId) : new Uint8Array();
  const data = concat2(version, zone, fromHex(address));
  return encode3(hrp, data);
}
function parse(tempoAddress) {
  let hrp;
  let data;
  try {
    const decoded = decode2(tempoAddress);
    hrp = decoded.hrp;
    data = decoded.data;
  } catch {
  ...
  let rawAddress;
  if (hrp === "tempoz") {
    const { value, size: size2 } = fromBytes3(payload);
    zoneId = value;
...
var InvalidLengthError2 = class extends BaseError2 {
